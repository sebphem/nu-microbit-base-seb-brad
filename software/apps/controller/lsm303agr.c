...
void lsm303agr_init(const nrf_twi_mngr_t *i2c)
{
  ...
  // treturns the same value if working
...
float _joystick_val(lsm303agr_measurement_t measurement_in)
{
  ...
  return (measurement_in.x_axis < 0 ) ? -1 * fabs(phi) : fabs(phi);
}


static void do_joystick_thing(void *_unused)
{
  // printf("start joystick measurement\n");
  joystick_val = _joystick_val(lsm303agr_read_accelerometer());
}



void init_joystick(){
  app_timer_create(&joystick_timer, APP_TIMER_MODE_REPEATED, do_joystick_thing);
  app_timer_start(joystick_timer, APP_TIMER_TICKS(100), NULL);
}